...
app.post("/projects/:replId/collaborators", checkJwt, async (req, res) => {
    ...
    try {
        ...
        if (!collaborator) return res.status(404).send("Collaborator user not found");
        if (collaborator.id === owner.id) return res.status(400).send("Owner is already a collaborator");
